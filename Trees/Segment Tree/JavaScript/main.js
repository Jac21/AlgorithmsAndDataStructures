...
class SegmentTree {
	...
	buildTreeRecursively(leftInputIndex, rightInputIndex, position) {
		...
		// split array into two halves and process them recursively
		...
		// once every tree leaf is not empty, we're able to build tree
		// bottom up using provided operation function
...
var run = function () {
	let array = [1, 2];
	let operation = Math.min;
	let segmentTree = new SegmentTree(array, operation, Infinity);
	console.log(`Segment Tree: ${segmentTree.segmentTree} with input array ${array} 
	and operation ${operation.toString()}`);
	console.log(`Segment Tree length: ${segmentTree.segmentTree.length}`);

	array = [-1, 2, 4, 0];
	operation = Math.min;
	segmentTree = new SegmentTree(array, operation, Infinity);
	console.log(`Segment Tree: ${segmentTree.segmentTree} with input array ${array} 
	and operation ${operation.toString()}`);
	console.log(`Segment Tree length: ${segmentTree.segmentTree.length}`);
	...
	array = [-1, 3, 4, 0, 2, 1];
	segmentTree = new SegmentTree(array, (a, b) => (a + b), 0);
	console.log(`Segment Tree: ${segmentTree.segmentTree} with input array ${array}`);
	console.log(`Segment Tree length: ${segmentTree.segmentTree.length}`);
	console.log(`Range query from 0 to 5: ${segmentTree.rangeQuery(0,5)}`);
}